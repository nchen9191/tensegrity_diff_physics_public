...
def run_target_gaits_pid(
    ...
    while (ctrls != 0.0).any():
        ctrls = pid.update_control_by_target_gait(
...
def init_sim(cfg,
    ...
    for i, c in enumerate(sim.tensegrity_robot.actuated_cables.values()):
        if rest_lengths is not None:
            act_len = c._rest_length - rest_lengths[:, i: i + 1] * 10  # scale
            c.actuation_length = act_len
        if motor_max_speed_pct is not None:
            c.motor.speed = torch.tensor([[[motor_max_speed_pct]]], dtype=torch.float64)
    ...
    if start_end_pts is None:
        # scale up
        start_state = scale_state(start_state, 10.)

        sim.update_state(start_state)
        start_end_pts = torch.vstack([e for r in sim.tensegrity_robot.rods.values() for e in r.end_pts])
    elif start_state is None:
        start_end_pts = 10 * start_end_pts  # scale up
        end_pts = start_end_pts.reshape(-1, 6, 1)  # scale up
        start_state = end_pts_to_start_state(end_pts)
        sim.update_state(start_state)

        # scale down
        start_state = scale_state(start_state, 0.1)

    start_end_pts = [
        [start_end_pts[2 * k].unsqueeze(0), start_end_pts[2 * k + 1].unsqueeze(0)]
        for k in range(start_end_pts.shape[0] // 2)
    ]
...
def scale_state(state, scale=10.):
    state_ = state.reshape(-1, 13, 1)
    state_[:, :3] *= scale
    state_[:, 7:10] *= scale
    state = state_.reshape(1, -1, 1)
    return state
...
def stabilize_robot(sim_fn,
    ...
    for _ in tqdm.tqdm(range(max_steps)):
        ...
        vel_mags = vels.norm(dim=1)
        max_vel = vel_mags.max()

        all_states.append(curr_state.clone())
    ...
    return stable_state, rest_lengths, motor_speeds, all_states
...
def align_prin_axis_2d(curr_end_pts, new_end_pts):
    ...
    new_robot_prin = new_end_pts[1::2].mean(dim=0, keepdim=True) - new_end_pts[::2].mean(dim=0, keepdim=True)
    new_robot_prin[:, 2] = 0.
    ...
    curr_robot_prin = curr_end_pts[1::2].mean(dim=0, keepdim=True) - curr_end_pts[::2].mean(dim=0, keepdim=True)
    curr_robot_prin[:, 2] = 0.
...
def run_robot_init_stabilization(
    ...
    all_states = []
    with torch.no_grad():
        init_end_pts = torch.vstack(batch_compute_end_pts(init_state.reshape(1, -1, 1), rod_length))

        # Run light stabilization
        curr_state, rest_lengths, motor_speeds, states = stabilize_robot(
            sim_fn,
            dt,
            init_state,
            init_rest_lengths,
        ...
        pid_params = [
            torch.tensor(pid_min_len, dtype=torch.float64),
            torch.tensor(pid_range, dtype=torch.float64),
            torch.tensor(pid_tol, dtype=torch.float64)
        ]
        rest_target_gaits = torch.tensor([1.0] * num_act_cables)
        states, rest_lengths, motor_speeds, _ = sim_fn.run_target_gaits(
            curr_state,
            dt,
            rest_lengths,
            motor_speeds,
        ...
        curr_end_pts = torch.vstack(batch_compute_end_pts(curr_state.reshape(1, -1, 1), rod_length))

        all_states.extend(states[::10])

        if vis:
            vis_states = [scale_state(s, 10.) for s in all_states]
            visualize(vis_states, dt, 'stabilize_vid.mp4')
            del all_states, vis_states

        # Realign robot
        start_state = align_prin_axis_2d(curr_end_pts, init_end_pts).flatten()

    return start_state, rest_lengths, motor_speeds
...
def edgar():
    ...
    base_path = Path(
        "/home/nelsonchen/research/tensegrity/data_sets/tensegrity_real_datasets/R2S2R/train/")
    j = -1
    for path in base_path.iterdir():
        ...
        start_end_pts /= 10.  # Unscale for Nelson's data
        rod_length = (start_end_pts[1] - start_end_pts[0]).norm().item()

        rest_lengths = torch.tensor([
            config['tensegrity_robot_config']['springs'][k]['rest_length'] - gt_extra_data[0]['act_lengths'][k]
            for k in range(len(gt_extra_data[0]['act_lengths']))
        ], dtype=torch.float64).reshape(1, 6, 1) + 0.4
        rest_lengths /= 10.  # Unscale for Nelson's data

        motor_speeds = torch.tensor([0., 0., 0., 0., 0., 0.],
                                    dtype=torch.float64).reshape(1, 6, 1)
        motor_max_speed_pct = 0.8
