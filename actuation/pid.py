...
class PID:
    def __init__(self,
        ...
        self.k_i = k_i
        self.k_d = k_d
        self.min_length = torch.tensor(min_length / 100., dtype=sys_precision)
        self.RANGE = torch.tensor(RANGE / 100., dtype=sys_precision)
        self.tol = torch.tensor(tol, dtype=sys_precision)
        self.LEFT_RANGE = None
        self.RIGHT_RANGE = None
        self.done = None

    def move_tensors(self, device):
        self.min_length = self.min_length.to(device)
        self.RANGE = self.RANGE.to(device)
        self.tol = self.tol.to(device)
    ...
    def update_control_by_target_gait(self, current_length, target_gait, rest_length):
        ...
        low_error_cond2 = torch.abs(current_length - target_length).flatten() < 0.01
    ...
    def update_control_by_target_gait_fn(self,
                                         current_length,
                                         target_gait,
                                         rest_length,
        ...
        u = torch.zeros_like(current_length)

        position = (current_length - self.min_length) / self.RANGE

        target_length = self.min_length + self.RANGE * target_gait
        error = position - target_gait
        ...
        low_error = torch.logical_or(
            torch.logical_or(done_flag.flatten(), low_error_cond1),
            torch.logical_or(low_error_cond2, low_error_cond3)
        )

        done_flag[low_error] = True

        d_error = (error - last_error) * first_step
        cum_error += error
        last_error = error

        u[~low_error] = (self.k_p * error[~low_error]
                         + self.k_i * cum_error[~low_error]
                         + self.k_d * d_error[~low_error])

        u = torch.clip(u, min=-1, max=1) * (~done_flag)

        slack = torch.logical_and(current_length < rest_length, u < 0)
        u[slack] = 0


        return u, last_error, cum_error, done_flag
