...
class SpringState(BaseStateObject):
    ...
    def __init__(self,
        ...
        self.stiffness = stiffness.reshape(1, 1, 1)
        self.damping = damping.reshape(1, 1, 1)
        self._rest_length = rest_length.reshape(1, 1, 1)
...
class ActuatedCable(Cable):
    ...
    def __init__(self,
                 ...
                 name: str,
                 winch_r: float,
                 min_winch_r: float = 0.01,
                 max_winch_r: float = 0.07,
                 sys_precision: torch.dtype = torch.float64,
                 motor: Optional[DCMotor] = None,
                 motor_speed: float = 0.99,
                 init_act_length: float = 0.0):
        ...
        self.init_act_length = torch.tensor(init_act_length, dtype=sys_precision).reshape(1, 1, 1)
        self.actuation_length = self.init_act_length.clone()
        self.dl = torch.tensor([[[0.]]], dtype=sys_precision)
    ...
    def _set_winch_r(self, winch_r: float):
